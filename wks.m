...
function [WKS,E,PHI,L,AREA] = wks(shape,params)
...
n_eigenvalues = params.neig; % number of eigenvalues used for computations
% depending on the application, you can use less than 300
N = params.N; % number of evaluations of WKS
wks_variance = params.variance; % variance of the WKS gaussian (wih respect to the 
...
if any(any(adjacency_matrix > 1))
    ...
    fprintf('Triangles must be oriented consistently.\n')
	%# beep
end
...
options.disp = 0;
[PHI,E] = eigs(L,Am,n_eigenvalues,-1,options);  %% default value -1e-5
E = diag(E);
E=abs(real(E));
[E,idx] = sort(E);
PHI = PHI(:,idx);
